...
function Notifications() {
    ...
    return (
        <div className="container my-5 bg-white p-4 rounded-3">
            ...
            <div className="notification-list">
                {notifications.length === 0 ? (
                    ...
                    notifications.map((notification, index) => (
                        <div
                            key={index}
                            className="d-flex align-items-center p-3 border-bottom hover-icon justify-content-between"
                            style={{
                                cursor: 'pointer',
                        ...
                        >
                            <div className="d-flex align-items-center">
                                <img src={shopInfo?.logo} alt="" height={80} width={80} className="me-3" />
                                <p className="fs-4 mb-1 text-wrap" style={{ maxWidth: '400px' }}>
                                    {notification.message}
                                </p>
                            </div>
                            <div className="d-flex justify-content-between">
                                <p className="text-muted">{notification.createdAt.toDate().toLocaleString('vi-VN')}</p>
                                {!notification.read && <span className="badge bg-primary rounded-pill">Mới</span>}
                            </div>
                        </div>
                    ))
                )}
