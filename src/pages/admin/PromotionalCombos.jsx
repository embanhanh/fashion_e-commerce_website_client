...
import {
    getPromotionalCombosAction,
    deleteManyPromotionalCombosAction,
...
function PromotionalCombos() {
                ...
                return (
                    (filter.status === '' ||
                        ...
                        (filter.status === 'ongoing' &&
                            new Date(combo.startDate) <= new Date() &&
                            new Date(combo.endDate) >= new Date()) ||
    ...
    const handleDeleteSelectedCombos = async (comboIds) => {
        if (comboIds?.length > 0) {
            try {
                await dispatch(deleteManyPromotionalCombosAction(comboIds)).unwrap()
                showNotification('Thành công', 'Xóa combo thành công')
                setSelectedCombos([])
                setBulkAction('')
            } catch (error) {
                showNotification('Thất bại', 'Xóa combo thất bại: ' + error.message, 'error')
            }
        }
    }
    ...
    useEffect(() => {
        if (bulkAction === 'deleteSelectedCombos' && selectedCombos.length > 0) {
            Swal.fire({
                title: 'Bạn có chắc chắn muốn xóa các combo này?',
                ...
                if (result.isConfirmed) {
                    handleDeleteSelectedCombos(selectedCombos)
                } else {
                    setBulkAction('')
                }
            })
            ...
            <div className="bg-white rounded-4 shadow-sm">
                ...
                <div className="d-flex p-3 justify-content-center align-items-center">
                    ...
                    <button
                        ...
                        onClick={() => setFilter({ name: '', startDate: null, endDate: null, status: '' })}
                    >
                    ...
                    <div className="d-flex align-items-center">
                        <div className="select ">
                            <div
                                ...
                                data-one="Xóa các combo đang chọn"
                            >
                                <svg
                            ...
                            <div className="options">
                                ...
                                <div title="option-1">
                                    ...
                                    <label
                                        className="option"
                                        htmlFor="option-1-v3"
                                        data-txt="Xóa các combo đang chọn"
                                    />
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="options">
                                ...
                                <div title="option-1">
                                    <input
                                        ...
                                        type="radio"
                                        value="upcoming"
                                        onChange={(e) => setFilter({ ...filter, status: 'upcoming' })}
                                    />
                                ...
                                <div title="option-2">
                                    <input
                                        ...
                                        type="radio"
                                        value="ongoing"
                                        onChange={(e) => setFilter({ ...filter, status: 'ongoing' })}
                                    />
                                ...
                                <div title="option-3">
                                    <input
                                        ...
                                        type="radio"
                                        value="ended"
                                        onChange={(e) => setFilter({ ...filter, status: 'ended' })}
                                    />
                        ...
                        <button
                            ...
                            onClick={() => navigate('/seller/combo/create')}
                        >
                            ...
                            <div className="checkbox-cell">
                                <label className="d-flex align-items-center">
                                    <input
                                        ...
                                        checked={selectedCombos.length === filteredPromotionalCombos.length}
                                ...
                                <div key={combo._id} className="combo-grid py-3 border-bottom">
                                    <div className="checkbox-cell">
                                        <label className="d-flex align-items-center">
                                            <input
                                                ...
                                                onChange={(e) => handleSelectCombo(e, combo._id)}
                                            />
                                            <span className="custom-checkbox"></span>
                                        </label>
                                    </div>
                                    ...
                                    <div className="overflow-y-auto scrollbar-y" style={{ maxHeight: '150px' }}>
                                        {combo.products.map((product) => (
                                            <div key={product._id} className="d-flex align-items-center my-2">
                                                ...
                                                />
                                                <p
                                                    className="ms-2 product-name fw-medium"
                                    ...
                                    <p
                                        className={`fs-4 fw-medium text-center ${
                                            new Date() >= new Date(combo.startDate) &&
                                    ...
                                    <div className="d-flex align-items-center flex-column">
                                        <FontAwesomeIcon
                                            ...
                                            color="#4a90e2"
                                            onClick={() => navigate(`/seller/combo/edit/${combo._id}`)}
                                        />
                                        <FontAwesomeIcon
                                            icon={faCircleInfo}
                                            className="fs-3 my-2 p-2 hover-icon"
                                            color="#000"
                                            onClick={() => handleShowDetailCombo(combo._id)}
                                        />
                                        <FontAwesomeIcon
                                            icon={faTrash}
                                            className="fs-3 my-2 p-2 hover-icon text-danger"
                ...
                <Modal show={showDetailCombo} onHide={handleCloseCombo} centered>
                    ...
                    <Modal.Body>
                        {status === 'loading' ? (
                            ...
                            <div
                                className="voucher-details p-3 border rounded overflow-y-auto"
